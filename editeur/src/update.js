function update()
{
    ...
    switch(gamePhase){
        ...
        case 'home':
            ...
            start.alpha = 1;
            betshot.alpha = 1;
            ground.setTint(0x60372,0x60372,0x953583,0x953583);
            break;
        ...
        case 'predeath':
            ...
            if(cars.getLength()!=0){
                ...
                //Score
                for(let k=0;k<score.toString().length;k++)
                    scoreGroup.create(config["width"]*.5+(k-score.toString().length*.5+.5)*widthEmpty*(1/3),config['height']*.21,score.toString().charAt(k)).setScale(scoreScale*globalScale*2/3);
                //BestScore
                for(let k=0;k<bestScore.toString().length;k++)
                    scoreGroup.create(config["width"]*.5+(k-bestScore.toString().length*.5+.5)*widthEmpty*(1/3),config['height']*.36,bestScore.toString().charAt(k)).setScale(scoreScale*globalScale*2/3);
